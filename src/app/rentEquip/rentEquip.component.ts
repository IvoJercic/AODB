...
@Component({
    ...
    templateUrl: './rentEquip.component.html',
    styleUrls: ['../bookTrip/bookTrip.component.css']
})

export class RentEquipComponent implements OnInit {
    ...
    kliknutaOpremaOstaleOpreme: string;
    klikuntiIznos: number;//Bitno -prosljedujem funkciji za instant ispisivanje cijene ostale opreme

    jedinicaVremena: string;

    kosaricaPenjackeOpreme = [];
    kosaricaOstaleOpreme = [];
    ukupnaCijenaPenjackeOpreme: number;//Odnosi se za jednu osobu u jednom danu
    ukupnaCijenaOstaleOpreme: number;
    unijaCijeneOprema: number;


    ngOnInit() {
        $("#popisPenjackeOpreme").hide();
    ...
    promijeniTipOpreme() {
        ...
        if ($('#opremaPenjanje').is(':checked')) {
            $("#popisPenjackeOpreme").show(250);
            $("#popisOstaleOpreme").hide(250);
            $("#prikazIzracunaOstaleOpreme").hide(250);
            this.unijaCijeneOprema = this.ukupnaCijenaPenjackeOpreme;
        }
        else {
            $("#popisPenjackeOpreme").hide(250);
            $("#popisOstaleOpreme").show(250);
            $("#prikazIzracunaOstaleOpreme").show(250);
    ...
    dodajPenjackuOpremuUKosaricu(element: string, cijena: number) {
    ...
    ukloniIzPenjackeKosarice(naziv: string, cijena: number) {
        let indeksUKosarici;
    ...
    ukloniIzOstaleKosarice(naziv: string, cijena: number) {
        let indeksUKosarici;
        ...
        if (this.kosaricaOstaleOpreme.length > 0) {
            $("#popisOstaleOpreme :button").attr("disabled", true);
        }
        else {
            $("#popisOstaleOpreme :button").attr("disabled", false);
        }
    }
    ...
    dodajOstaluOpremuUKosaricu(naziv: string, cijena: number, vremenskaJedinica: string) {
        this.kliknutaOpremaOstaleOpreme = naziv
        this.klikuntiIznos = cijena;
        this.trajanje = $("#trajanjeUnos").val();
        ...
        if (cijena == 0) {
            ...
            $("#trajanjeUnos").val(1);
            $("#trajanjeUnos").attr('disabled', true);
        }
        else {
            $("#trajanjeUnos").attr('disabled', false);
    ...
    osvjeziCijenu() {
        ...
        if ($('#opremaPenjanje').is(':checked')) {
            let temp: number = this.ukupnaCijenaPenjackeOpreme * this.trajanje * this.kolicina;
            //console.log(temp);            
            this.unijaCijeneOprema = temp;
            $("#zaPlatitiUnos").val(temp - this.placeno);
            ...
            this.kolicina = $("#kolicinaUnos").val();
            this.trajanje = $("#trajanjeUnos").val();
            $("#zaPlatitiUnos").val(this.ukupnaCijenaOstaleOpreme - this.placeno);
            this.unijaCijeneOprema = this.ukupnaCijenaOstaleOpreme;
        }
        //console.log(this.trajanje+" x "+this.kolicina+" x "+ this.ukupnaCijenaPenjackeOpreme);

        $("#ukupnoUnos").val(this.unijaCijeneOprema);
    }
}